window.onload = function() {
	...
	template.helper('noteImg',function(obj){
		if(obj!=null && obj!=""){
			...
			return '<div class="aimg" style="background-image:none;"><img   src="img/defaultHeadImg.jpg"/></div>';
		}
	})
	//课程信息
	var courseMess =
	...
	RequestService("/online/user/isAlive", "GET", null, function(data) { ///online/user/isAlive
		if(data.success === true) {
			RequestService("/profession/findEnterCourseDetail", "get", {
				courseId: courseID
			}, function(data) {
				if(data.success == true) {
					$(".content .bread .cur").html(data.resultObject.courseName);
					$(".content .profession-mess").html(template.compile(courseMess)({
						items: data.resultObject
					}));
					$(".content .profession-range").append(template.compile(courserange)({
						result: data.resultObject
					}));
					document.title = data.resultObject.courseName + " - 博学谷云课堂";
					//0职业课，1微课
					if(data.resultObject.course_type==0){
						$(".profession-nav .jihua").show();
						$(".profession-main .main1").show().siblings().hide();
						plan();
					}else{
						$(".profession-nav .jihua").hide();
						$(".profession-nav .video").addClass("active");
						$(".profession-main .main").show().siblings().hide();
						nav();
					};
					//免费课程没有学习计划
					...
					//继续学习
					$(".content .profession-range .l3").off("click").on("click", function() {
						$.ajax({
							type: "get",
							url: bath + "/online/user/isAlive",
							async: false,
...
//						} else 
			...
			//目录评价导航切换
			$(".profession-nav li").on("click", function() {
				isAlive();
				...
				function calder(year, num, rest) {
					...
					$(".calder ul .noban").off().on("click", function() {
						if(!$(this).hasClass("ban")) {
							...
							$("body").animate({"scrollTop":$(".profession-nav").get(0).offsetTop},200);
						}
					});
			...
			function nav() {
				RequestService("/online/chapter/findChapterInfo", "get", {
					...
					if(data.success == true) {
						$(".profession-main .main-left").html(template.compile(coursenav)({
							result: data.resultObject
						}));
						$(".profession-main .main-right .main-video").show().siblings().hide();
			...
			//课程表格列表接口函数				
			function course(id) {
				$(".profession-main .main-right .main-video .right-con").html('<div class="con-wait">' +
				...
				RequestService("/online/chapter/findChapterByChapterId", "get", {
					...
					$(".profession-main .main-right .main-video .right-con").html(template.compile(coursetable)({
						items: data.resultObject
					}));
			...
			function info() {
				...
				RequestService("/profession/resource/getChapterTree", "get", {
					courseId: courseID
				}, function(data) {
					if(data.success == true) {
						$(".profession-main .main-left").html(template.compile(infonav)({
							result: data.resultObject
						}));
						$(".profession-main .main-right .main-ppt").show().siblings().hide();
						//伸缩效果加载
			...
			function Notes() {
				...
				RequestService("/video/getvideos", "get", {
					...
					if(data.success == true) {
						$(".profession-main .main-left").html(template.compile(notes)({
							result: data.resultObject
						}));
						$(".profession-main .main-right .main-note").show().siblings().hide();
						//伸缩效果加载
						$(".main-left .allinfo .info1-name").off().on("click", function() {
							if($(this).hasClass("act")) {
								$(this).removeClass("act").next().addClass("hide");
							...
							function findNotes() {
								RequestService("/notes/findNotes", 'GET', {
									...
									if(data.resultObject.items.length == 0) {
										$(".profession-main .main-right .main-note .right-con").html(template.compile(emptyDefaul));
										$(".page-no-result").show();
									} else {
										$(".profession-main .main-right .main-note .right-con").html(template.compile(noteContent)({
			...
			function wenda() {
				...
				RequestService("/video/getvideos", "get", {
					...
					if(data.success == true) {
						...
						$(".profession-main .main-right .main-ask").show().siblings().hide();
						//伸缩效果加载
						$(".main-left .allinfo .info1-name").off().on("click", function() {
							if($(this).hasClass("act")) {
								$(this).removeClass("act").next().addClass("hide");
						...
						$(".main-left .allinfo .info2").off().on("click", function() {
							...
							function askAndQus() {
								RequestService('/online/questionlist/findVideoQuestion', 'GET', {
									...
									if(data.resultObject.items.length == 0) {
										$(".profession-main .main-right .main-ask .right-con").html(template.compile(emptyDefaul));
										$(".page-no-result").show();
									} else {
										$(".profession-main .main-right .main-ask .right-con").html(template.compile(askContent)({
											items: data.resultObject.items
										}));
									}
									showAndHide();
