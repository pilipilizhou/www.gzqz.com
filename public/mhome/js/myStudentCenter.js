...
$.getUrlParam = function(name) {
	...
	var mycourse = '<div class="box clearfix">' +
		...
		'<div class="profession" data-url="{{#zhibohref($value.id)}}">' +
	...
	var exam1 =
		...
		'<span class="profession" title="{{$value.courseName}}">{{$value.courseName}}</span>' +
	...
	var work1 =
		...
		'<span class="profession" title="{{$value.name}}">{{$value.name}}</span>' +
	...
	RequestService("/online/user/isAlive", "GET", null, function(data) { ///online/user/isAlive
		if(data.success === true) {
			...
			function course(courseStatus, pageNumber, pageSize) {
				isAlive();
				$(".main-right .mycourse .right-profession .loadrr").show().siblings().hide();
					...
					if(data.success == true) {
						if(data.resultObject.items == "") {
							if(data.resultObject.totalCount == 0) { //无数据
								$(".main-right .mycourse .right-profession .course" + courseStatus).html(emptyDefaul);
								$(".main-right .mycourse .right-profession .course" + courseStatus).show().siblings().hide();
								$(".pages").css("display", "none");
							}
							...
							$(".main-right .mycourse .right-profession .course" + courseStatus).show().siblings().hide();
							if(courseStatus == 0) {
								$(".main-right .mycourse .right-profession .course0").html(template.compile(mycourse)({
									items: data.resultObject.items
								}));
							} else if(courseStatus == 1) {
								$(".main-right .mycourse .right-profession .course1").html(template.compile(mycourse)({
									items: data.resultObject.items
								}));
							}
						...
						$(".content .main-right .mycourse .right-title span").off().on("click", function() {
							...
							if($(this).attr("kg") == "true") {
								...
								if($(this).attr("data-id") == 0) {
									$(".main-right .mycourse .right-profession .course0").show();
									$(".main-right .mycourse .right-profession .course1").hide();
								} else if($(this).attr("data-id") == 1) {
									$(".main-right .mycourse .right-profession .course1").show();
									$(".main-right .mycourse .right-profession .course0").hide();
								};

							};
						});
						//点击课程
						$(".right-profession .profession").off().on("click",function(){
							var $this=$(this);
							$.ajax({
								type: "get",
								url: bath + "/online/user/isAlive",
						...
						$(".main-right .mycourse .right-profession").html(emptyDefaul);
						$(".pages").css("display", "none");
					}
				})
			};
			...
			function order(state, time, pageNumber, pageSize) {
				RequestService("/web/getMyAllOrder", "get", {
					...
					if(data.success == true) {
						if(data.resultObject.items == "") {
							if(data.resultObject.totalCount == 0) { //无数据
								$(".mybuy .right-profession .ordertable").html(emptyDefaul);
								$(".pages").css("display", "none");
							}
						} else if(data.resultObject.totalPageCount == 1 && data.resultObject.totalCount > 0) {
							$(".pages").css("display", "none");
							$(".mybuy .right-profession .ordertable").html(template.compile(myorder)({
								items: data.resultObject.items
							}))
						} else if(data.resultObject.totalPageCount > 1) {
							$(".pages").css("display", "block");
