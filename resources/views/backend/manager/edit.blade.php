...
<article class="page-container">
	<form class="form form-horizontal" id="form-admin-add" action="{{ url('System/Manager/Save') }}/{{$mg_id}}" method="post">
	{{csrf_field()}}
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>管理员：</label>
		<div class="formControls col-xs-8 col-sm-9">
			{{--把id和name属性去掉，免得提交--}}
			<input type="text" class="input-text" value="{{$manager->username}}" readonly  placeholder="" STYLE="background: #ccc;border: 1px solid #ccc; color:#ebebeb;">
		</div>
	</div>
	{{--<div class="row cl">--}}
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="mg_role_ids" size="1">
			 	 @foreach($roles as $role)
					@if($manager->mg_role_ids == $role['role_id'])
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<textarea name="mg_remark" cols="" rows="" class="textarea"   placeholder="说点什么...100个字符以内" dragonfly="true" onKeyUp="$.Huitextarealength(this,100)">{{$manager->mg_remark or ''}}</textarea>
