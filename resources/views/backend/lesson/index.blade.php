...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> <a href="javascript:;" onclick="top.location.href='/System/Home/Index'">首页</a> <span class="c-gray en">&gt;</span> 课时模块管理 <span class="c-gray en">&gt;</span> 课时列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray"> <span class="l"> <!--a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a--> <a class="btn btn-primary radius" href="javascript:;" onclick="lesson_add('添加课时','{{ url('System/Lesson/Add') }}','800')"><i class="Hui-iconfont">&#xe600;</i> 添加课时</a> </span></div>
...
<table class="table table-border table-bordered table-hover table-bg list-data">
		<thead>
			<tr>
				<th scope="col" colspan="8">课时列表</th>
			</tr>
			<tr class="text-c">
				<th width="25"><input type="checkbox" value="" name=""></th>
				<th width="40">ID</th>
				<th width="200">课时名称</th>
...
<script>
	...
	$('.list-data').DataTable({
		...
		// ajax获取服务端的数据
		"ajax": {
		    "url": "{{ url('System/Lesson/ApiList') }}", // ajax的请求地址
		    "type": "POST",
    	...
    	{'data':'profession.course_name',"defaultContent": "",'className':"text-c"},
		...
		"createdRow": function(row,data,index){
			...
			if(data.cover){
			    $(row).children().eq(4).html('<img src="{{config('program.url')}}/'+data.cover+'"style="width:100px;height:60px;">');
			}else{
			    $(row).children().eq(4).html("暂无封面图片");
			}
			$(row).children().eq(-1).html(`<a title="编辑" href="javascript:;" onclick="lesson_edit('编辑课时信息','{{url('/System/Lesson/Edit')}}/`+ data.id +`' ,'`+ data.id +`')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
				<a title="删除" href="javascript:;" onclick="lesson_del(this,`+data.id+`)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>`);
		},
	});
	/*课时-添加*/
	...
	function lesson_del(obj,id){
		layer.confirm('您确认要删除该课时吗？',function(index){
			$.ajax({
				type: 'POST',
				url: '{{ url('System/Lesson/Remove') }}/'+id,
				dataType: 'json',
				data: {
					_token:'{{ csrf_token() }}',
				},
