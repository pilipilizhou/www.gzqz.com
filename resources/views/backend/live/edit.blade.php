...
@section('content')
	...
	<article class="page-container">
		<form action="{{ url('System/Live/Save') }}/{{$id}}" method="post" class="form form-horizontal" id="live-save">
			{{ csrf_field() }}
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>课程名称：</label>
				<div class="formControls col-xs-4 col-sm-6">
					<input type="text" class="input-text" name="course_name" value="{{$live->course_name}}" id="course_name">
				</div>
			</div>
			<div class="row cl">
          ...
          <select class="select" name="stream_id" id="stream_id">
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-6">
                    <script id="course_desc" name="course_desc" type="text/plain" style="width:540px;height:260px;">
                        {!! $live->course_desc !!}
                    </script>
                </div>
            </div>
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<!-- 保存上传成功以后的图片地址 -->
					<input type="hidden" name="cover" value="{{$live->cover}}">
					<!-- 定义一个标签用来存放图片的预览图 -->
					<div id="webuploader-img" style="margin-bottom: 5px;">
						@if($live->cover)
							<img src="{{config('program.url')}}/{{$live->cover}}" style="width:150px;height: 90px;">
						@else
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-4 col-sm-6">
					<input type="text" class="input-text" name="sort" value="{{$live->sort}}" id="sort" placeholder="数值越大，越靠前！">
				</div>
			</div>
			<div class="row cl">
